...
초기 MVP는 `admate-homepage`에 임원 보고용 시각 화면으로 구현하고, 데이터는 간단한 JSON 또는 설정 파일로 관리한다. 이후 운영 입력, 이력 관리, 권한, audit log가 필요해지면 `openclaw-monitor`의 Agent Core 운영 콘솔로 확장한다.
...
이 항목들은 Phase 2 이후 `openclaw-monitor`에서 운영형 대시보드로 확장한다.
...
초기 구현은 admate-homepage, 운영형 확장은 openclaw-monitor가 담당한다.
```
